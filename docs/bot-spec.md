...
- **単語(word)**:
  ...
  - 末尾に `🔗` Field(辞書リンク)。
  - Embed と同じメッセージに、ユニーク見出し語ごとの発音 mp3(`{headword}.mp3`)を添付。target_lang(`en` / `ja`)で gTTS が読み上げる。個別 TTS 失敗はその見出し語のみスキップし他は継続。
- **文章(sentence)**:
...
- 実行時刻: 毎週土曜 09:00(Asia/Tokyo)。
- 実行主体: APScheduler が Bot プロセス内で動作。
- 投稿先: 専用チャンネル(`.env` の `REPORT_CHANNEL_ID` で指定)。
- 集計単位: 学習方向(`target_lang` = en or ja)で分けて投稿。Bot 1 つにつき 1 学習方向分のみ。
- 集計期間: **発火時点から遡る 7 日間(rolling 7-day window)**。土曜 09:00 発火 → 前週土曜 09:00 〜 当週土曜 09:00 を半開区間で集計。
...
[📥 単語をCSVでエクスポート（Anki用）]   ← 永続ボタン
```

- 学習方向(`target_lang`)ごとに 1 Embed。Bot は自分の `target_lang` に該当する 1 件のみ投稿。
- 上部に 3 フィールド並びのダッシュボード(質問数 / 学習日数 / クイズ正解率)。クイズ未実施なら「未実施」と表示。
- 続けて 3 セクション(word / sentence / grammar)を持つが、0 件のセクションは省略。
- 1 セクション内が 50 件を超える場合は上位 50 件まで表示し、残件数を併記。word セクションでオーバーフロー時は「全件はCSVをダウンロード」と注記。
- Embed と同時に永続 View(`WeeklyCsvView`)を貼る。詳細は 2.3。

### 2.3 Anki CSV エクスポート(永続ボタン)

- ボタン: `📥 単語をCSVでエクスポート（Anki用）`(`discord.ButtonStyle.primary`、`custom_id = weekly_csv:{target_lang}:{start_date}`)。
- 永続化: Bot 再起動を跨いでも反応するように `timeout=None` で生成し、`main.on_interaction` 経由でルーティング(`reports/weekly_view.parse_custom_id`)。
- 動作: 押下した瞬間に当該週の word 行を集計して Anki Basic 形式 CSV を生成し、ephemeral で添付返信。
- CSV 形式: ヘッダに `#separator:Comma` / `#html:false` / `#notetype:Basic` / `#columns:Front,Back` / `#deck:{deck_label} (YYYY-MM-DD)`。本文は `Front,Back` の 2 列。
  - Front: target_lang 側(漢字を含む見出し語は `単語【よみ】`)。
  - Back: explanation_lang 側の訳語(`result_summary`、または逆引き時は元の `query_text`)。
- 整形ルール(`reports/anki_card.build_anki_cards`):
  - word kind かつ target_lang 一致行のみ対象。
  - `result_summary` が `該当なし` / `無意味` / `no match` / `n/a` / 空白なら除外。
  - Mode A(`query_text` が target_lang スクリプト): そのまま Front。
  - Mode B(`query_text` が explanation_lang スクリプト): `result_summary` を ` / ` で分割し、target_lang スクリプトに一致する各見出し語を Front 側へ転置。
  - Front 完全一致は最初の 1 件のみ採用(出現順保持)。
- カード 0 件の週は「この週には単語の記録がありませんでした。」を ephemeral で返す。
- ファイル名: `weekly-words-{target_lang}-{YYYY-MM-DD}.csv`。
